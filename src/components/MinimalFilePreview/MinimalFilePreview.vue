...
<template>
<ul role="list" class='grid | minimal-file-preview' >
    
    <li class="flex | minimal-file-preview__item" v-for="file in files" :key="file.id">
        <img v-if="checkFileType(file.file)" class="minimal-file-preview__file" :src="file.url" :alt="file.name">
        <video muted loop @mouseenter="onMouseEnter" @mouseleave="onMouseLeave" v-else class="minimal-file-preview__file">
            <source :src="file.url" >
        </video>
        <MinimalButton @click="onRemoveButtonClick(file)" data-circle="true" class="minimal-file-preview__button" aria-label="Delete file from list">X</MinimalButton>
    </li>
...
<style lang='scss' scoped>
 .minimal-file-preview {
    ...
    &__file {
        border: var(--file-preview-border-width, 4px) solid var(--file-preview-border-color, var(--clr-neutral-900));
